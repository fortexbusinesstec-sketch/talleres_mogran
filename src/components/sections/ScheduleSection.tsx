...
export function ScheduleSection() {
  ...
  return (
    <section
      id="schedule"
      className="bg-mogran-white section"
    ...
    >
      <div className="container-section">
        ...
        <div className="flex justify-center mb-10" role="tablist" aria-label="Modalidad de clase">
          <div className="inline-flex bg-white rounded-full p-1 border border-mogran-border">
            <button
              role="tab"
              aria-selected={modalidad === 'virtual'}
              aria-controls="schedule-content"
              onClick={() => setModalidad('virtual')}
              className={`px-6 py-2.5 text-sm font-medium rounded-full transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-mogran-primary focus-visible:ring-offset-2 focus-visible:ring-offset-mogran-white ${
                modalidad === 'virtual'
                  ? 'bg-mogran-primary text-white shadow-sm font-semibold'
                  : 'text-mogran-neutral hover:text-mogran-secondary hover:bg-mogran-tertiary'
              }`}
            >
              Virtual
            </button>
        ...
        {modalidad === 'virtual' && (
          ...
          >
            <div className="inline-flex items-center gap-6 text-mogran-neutral bg-mogran-primary/10 border border-mogran-primary/20 px-6 py-3 rounded-2xl flex-wrap justify-center">
              <span className="flex items-center gap-2">
                <IconClock size={18} strokeWidth={2} aria-hidden="true" />
                8 horas por mes
              </span>
              <span className="w-px h-4 bg-mogran-primary/30" aria-hidden="true" />
              <span className="flex items-center gap-2">
        ...
        {modalidad === 'presencial' && (
          ...
          >
            <div className="bg-mogran-primary/10 border border-mogran-primary/20 rounded-2xl p-6 flex items-start gap-4">
              <div
                className="flex-shrink-0 w-12 h-12 rounded-full bg-white flex items-center justify-center"
                aria-hidden="true"
              >
                <IconMapPin size={24} strokeWidth={1.5} className="text-mogran-primary" />
              </div>
              <div>
                <h3 className="text-lg font-semibold text-mogran-secondary mb-1">Única sede</h3>
                <p className="text-mogran-neutral">
        ...
        >
          {horarios.map((horario, index) => (
            <article
              key={index}
              className="bg-white border border-mogran-border rounded-xl p-6 transition-all duration-300 hover:border-mogran-primary/30 hover:shadow-lg hover:shadow-mogran-primary/5"
            >
              <Badge variant="primary" size="sm" className="mb-3">
                {horario.publico}
              </Badge>
              <p className="text-sm text-mogran-neutral/60 mb-3">{horario.rango}</p>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-mogran-secondary">
                  <IconClock size={16} strokeWidth={2} className="text-mogran-primary" aria-hidden="true" />
                  <span className="text-sm font-medium">{horario.dias}</span>
                </div>
                <p className="text-mogran-neutral text-sm pl-6">{horario.hora}</p>
                {horario.nota && (
